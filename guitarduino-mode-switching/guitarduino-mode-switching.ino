...
byte modeSwtiches = 0; //variable to store mode switch setting
...
void setup()
{
  ...
  DDRD = B00000010; //sets Port D to input
}


void loop()
{
  modeSwitches = PIND; //reads the mode switches on Port D
  modeSwitches = modeSwitches >> 2;
  swtich (modeSwitches)
    {
    //case B0000:
    //
